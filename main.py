...
class Player:  # Each player has their own gameInstance object created when calling $start
    ...
    ADMIN_LIST = {'Burbot#5573', 'ColdBrewOnNitroStat#4666', 'Lucas J#3567',
                  '24karatsunshine#7559'}  # List stored as Set, Set is pretty much better/efficient list if order does
    # not matter.
    PORTFOLIO_PRESET = [
    ...
    def __init__(self, name, age=18):  # __init__ is a object method that python automatically runs, when the
        ...
        # Setting to empty one to begin with.

        self._previous_assets = dict()  # Storing all of the player's data as key:pair dictionary format.
        # Setting to empty one to begin with.

        self._current_allocations = tuple()  # 9 asset types stored as tuple (a list that you cant edit individual
