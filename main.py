...
class player:                    #Each player has their own gameInstance object created when calling $start
    ...
    def __init__(self, name, age=18):        #__init__ is a object method that python automatically runs, when the instance/object is initialized/created
        ...
        self._is_admin = self._is_admin_check()    #Boolean; states whether or not the player have admin privileges, uses function to check if the player's name is in the class list of admins.
    ...
    def portfolio(self, preset_number):         #Bot command $Portfolio [#] would run this 
        index = preset_number - 1               #Python uses Based 0 index, but we want to let users start from Index of 1. So accounting internally.
        self._current_allocations = player.PORTFOLIO_PRESET[index]
        print("Portfolio Successfully Changed.")
        self.allocations = f"MONEY: {self._current_allocations[0]*100}%, ITGVT: {self._current_allocations[1]*100}%, LTCORP: {self._current_allocations[2]*100}%, Equity US: {self._current_allocations[3]*100}%, INT: {self._current_allocations[4]*100}%, SMALL: {self._current_allocations[5]*100}%, AGGR: {self._current_allocations[6]*100}%, FIXED: {self._current_allocations[7]*100}%, BALANCED: {self._current_allocations[8]*100}%"
